...
// Versão 0.4 - Relacionamentos + Filtros + Navegação encadeada
...
function render(tipo) {
  ...
  dados[tipo].forEach((item, i) => {
    ...
    // navegação encadeada: se houver relacionamento saindo desta entidade, adiciona click
    ${relacionamentos.map(rel => `
    if (tipo === '${rel.de}') {
      li.onclick = () => navegarEncadeado('${rel.para}', '${rel.via}', item['id']);
...
console.log('✅ App com navegação encadeada gerado em ./dist/index.html');
